...
def get_pod_node(pod_name, namespace):
    # Use 'oc get pods' to retrieve all pods in the namespace
    command = ["get", "pods", "-n", namespace, "-o", "json"]
    pods_json = run_oc_command(command)

    # Check if pods_json is valid and contains the expected pod names
    import json

    pods = json.loads(pods_json)

    # Loop through the pods to find one that matches the expected naming pattern
    for pod in pods.get("items", []):
        if (
            pod["metadata"]["name"].startswith(pod_name)
            and "-head-" in pod["metadata"]["name"]
        ):
            node_name = pod["spec"]["nodeName"]
            if node_name:
                return node_name

    raise ValueError(f"Could not retrieve node for pod {pod_name}")
